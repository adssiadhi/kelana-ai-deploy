    ...
    <section id="destinations" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold bg-neutral-200 text-neutral-700 border border-neutral-300 mb-2 clip-chamfer-sm">
            <Sparkles className="w-3.5 h-3.5 text-neutral-500" /> Curated Destinations
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {FEATURED_DESTINATIONS.map((dest) => (
          <div
            key={dest.name}
            className="group relative bg-white border border-neutral-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col clip-chamfer-lg overflow-hidden"
          >
            <div className="relative h-56 w-full overflow-hidden bg-neutral-100">
              ...
              <div className="absolute top-3 left-3">
                <span className="inline-flex items-center px-2.5 py-1 text-xs font-bold bg-white/90 text-neutral-900 backdrop-blur-md shadow-sm clip-chamfer-sm">
                  {dest.tag}
                </span>
              </div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
              <p className="text-xs text-neutral-600 leading-relaxed">{dest.description}</p>
              ...
              <div className="pt-2 border-t border-neutral-100 flex items-center justify-between">
                ...
                <button
                  onClick={() => onSelectDestination(`${dest.name}, ${dest.country}`, dest.suggestedDays, dest.estimatedBudget, dest.category)}
                  className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-neutral-700 bg-neutral-100 hover:bg-neutral-800 hover:text-white transition-all group-hover:bg-neutral-800 group-hover:text-white clip-chamfer-sm"
                >
                  <span>Plan Trip</span>
